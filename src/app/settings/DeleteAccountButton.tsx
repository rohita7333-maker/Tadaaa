  ...
  return (
    <div className="space-y-3">
      ...
      <input
        ...
        placeholder="DELETE"
        className="w-full h-10 px-4 rounded-xl border border-[#E9E6DF] text-sm outline-none focus:border-red-400 transition-colors"
      />
      {error && <p className="text-xs text-red-600">{error}</p>}
      <div className="flex gap-3">
        <button
          onClick={() => { setOpen(false); setValue(""); }}
          className="flex-1 h-10 rounded-full border border-[#E9E6DF] text-[#6F6E68] text-sm hover:bg-[#FFF0E8] transition-colors"
        >
          Cancel
        </button>
        <button
