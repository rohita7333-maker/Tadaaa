/**
 * config.ts — Scroll Story reveal configuration.
 *
 * Phase 1 renders exclusively from `demoConfig`; Phase 2 maps real invite
 * rows into `StoryConfig`.
 */
...
export interface StoryConfig {
  /** Seed for deterministic particle layout (SSR-safe). */
  slug: string;
  recipient: string;
  eyebrow: string;
  occasionLine: string;
