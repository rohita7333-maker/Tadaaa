...
describe("templates data", () => {
  ...
  it("every occasionId exists in occasions", () => {
    ...
    for (const template of templates) {
      expect(occasionIds, `template "${template.id}" occasion`).toContain(
        template.occasionId
      );
    }
  });
  ...
  it("every themeId exists in themes", () => {
    ...
    for (const template of templates) {
      expect(themeIds, `template "${template.id}" theme`).toContain(
        template.themeId
      );
    }
  });
  ...
  it("tier matches the referenced theme's isPremium", () => {
    for (const template of templates) {
      ...
      expect(theme, `template "${template.id}" theme lookup`).toBeDefined();
      expect(template.tier, `template "${template.id}" tier`).toBe(
  ...
  it("every template has 1-3 non-empty style tags", () => {
    for (const template of templates) {
      expect(
        template.styleTags.length,
        `template "${template.id}" styleTags count`
      ).toBeGreaterThan(0);
      expect(
        template.styleTags.length,
        `template "${template.id}" styleTags count`
      ...
      for (const tag of template.styleTags) {
        expect(tag.trim(), `template "${template.id}" styleTag`).toBe(tag);
        expect(tag.length, `template "${template.id}" styleTag`).toBeGreaterThan(0);
      }
    }
  });
  ...
  it("every template has a valid paletteTag", () => {
    for (const template of templates) {
      expect(VALID_PALETTE_TAGS, `template "${template.id}" paletteTag`).toContain(
        template.paletteTag
      );
    }
  });
  ...
  it("golden-hour reads as a birthday template, not a date-invite one", () => {
    // Regression guard: the tagline is a birthday surprise ("sunset light for
    // the birthday..."), so the occasion must agree with it. See registry
    // comment in templates.ts for the full rationale.
...
describe("filterTemplates", () => {
  ...
  it("ORs multiple style tags together", () => {
    ...
    expect(ids).toContain("after-dark"); // moody
    expect(ids).toContain("shoreline"); // coastal
    for (const t of result) {
      expect(t.styleTags.some((tag) => ["moody", "coastal"].includes(tag))).toBe(
        true
      );
    }
  });
