// Ported 1:1 from web src/lib/themes.ts. Theme + occasion registries must stay
// identical to the web app so an invite created on either renders the same.
//
// Web stores `background` as a CSS linear-gradient string. RN can't consume that
// directly, so gradientStops() extracts the hex stops for expo-linear-gradient.
...
export const themes: Theme[] = [
  {
    ...
    colors: {
      ...
      backgroundSecondary: "#FFD8CC",
      text: "#2A1518",
      accent: "#B33A45",
      accentLight: "#F08C92",
      overlay: "rgba(42,21,24,0.45)",
    },
    fonts: { heading: "Bricolage Grotesque", body: "DM Sans" },
    revealIcon: "envelope",
    particleType: "hearts",
  ...
  },
  {
    ...
    price: 0,
    colors: {
      background: "linear-gradient(135deg, #FFE9C2 0%, #FFC97A 55%, #E89A3E 100%)",
      backgroundSecondary: "#FFE0AA",
      text: "#2A1A05",
      accent: "#B6802A",
      accentLight: "#F0C76A",
      overlay: "rgba(42,26,5,0.45)",
    },
    fonts: { heading: "Bricolage Grotesque", body: "DM Sans" },
    revealIcon: "gift",
...
/**
 * Extract the ordered hex color stops from a theme's CSS linear-gradient string
 * for use as expo-linear-gradient `colors`. Falls back to a 2-stop cream ramp.
 */
export function gradientStops(theme: Theme): string[] {
  const matches = theme.colors.background.match(/#[0-9a-fA-F]{3,8}/g);
  if (matches && matches.length >= 2) return matches;
  return [theme.colors.backgroundSecondary, theme.colors.background];
}
...
export const occasions: Occasion[] = [
  ...
  },
  {
    ...
    label: "Apology",
    emoji: "🙏",
    prompts: ["Can we start over?", "I'm sorry. Can we talk?", "Will you forgive me?"],
  },
  { id: "custom", label: "Custom", emoji: "💌", prompts: [] },
];
